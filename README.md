...
| `commercial-contract`（商事合同） | 合同全生命周期 | 7 | 合同起草（六步法） · 合同审查（五阶段法/速查表/A·B·C 方案） · **口径自学习闭环**（签约复盘→偏离日志→监控提案→审批落地） |
| `dispute-resolution`（商事争议） | 诉讼/仲裁全流程 | 19 | 全流程编排 · 拆案 · 案情分析 · 大事记 · 起诉/答辩/上诉/再审/代理词 · 判决拆解 · 再审分析 · 可视化 · 律师函 · 和解 · **要件分析表** · **组合视图** · **期限台账+监控代理** |
...
两个监控代理：
- **deadline-watcher**（dispute-resolution）——跟踪开庭/举证/答辩/上诉/再审/申请执行/保全续封/仲裁时效，期限届满前出"决策点"预警（建议每周运行，临庭每日）。
- **playbook-monitor**（commercial-contract）——盯偏离日志，同一条款 12 个月偏离满阈值即起草口径更新提案，律师审批后落地；绝不自动改 playbook。
