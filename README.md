...
| `dispute-resolution`（商事争议） | 诉讼/仲裁全流程 | 17 | 全流程编排 · 拆案 · 案情分析 · 大事记 · 起诉/答辩/上诉/再审/代理词 · 判决拆解 · 再审分析 · 可视化 · 律师函 · 和解 · **期限台账+监控代理** |
...
`dispute-resolution` 另含 **deadline-watcher 期限监控代理**：跟踪开庭/举证/答辩/上诉/再审/申请执行/保全续封/仲裁时效，期限届满前出"决策点"预警（建议每周运行，临庭每日）。
